...
public:
    ...
    find(const key_type& key, mapped_type& val, uint64_t& val_addr = dummy_val_addr) const {
        size_t hv = hashed_key(key);
        auto b = snapshot_and_lock_two(hv);
        const cuckoo_status st = cuckoo_find(key, val, hv, b.first, b.second, val_addr);
...
private:
    ...
    try_read_from_bucket(const partial_t partial, const key_type &key,
                         mapped_type &val, const size_t i, uint64_t& val_addr = dummy_val_addr) const {
    ...
    cuckoo_find(const key_type& key, mapped_type& val,
                const size_t hv, const size_t i1, const size_t i2, uint64_t& val_addr = dummy_val_addr) const {
